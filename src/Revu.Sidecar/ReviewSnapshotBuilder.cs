        ...
        long gameId, PromptClipContext promptClips)
    {
        ...
        try
        {
            ...
            foreach (var obj in objectives)
            {
                ...
                // `if (!label) continue`) never renders them, so listing one here would
                // orphan any clip tagged to it (reachable via the R-002 champ-select
                // if-then authoring). Excluding it keeps C# and JS in lockstep so its
                // clips fall into "To sort".

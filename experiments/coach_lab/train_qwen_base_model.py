...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare or train the shared Qwen base judge for Coach Lab."
    )
    parser.add_argument("--input", type=Path, default=DEFAULT_EXPORT / "moments.jsonl")
    parser.add_argument("--output", type=Path, default=MODELS_ROOT / "qwen-base")
...
def main() -> None:
    ...
    if len(training_records) < args.min_examples:
        raise SystemExit(
            f"Need at least {args.min_examples} accepted manual clips before preparing the base judge."
        )

    model_version = version_name("qwen-base-judge")
    ...
    config_path = model_root / "axolotl.base.yml"
    ...
    if trained and args.register:
        register_model(
            ...
            ModelRecord(
                ...
                model_kind="qwen_base",
                display_name=f"Qwen Base Judge ({dataset_info['train_count']} clips)",
